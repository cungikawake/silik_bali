...
<div class="main-body">
	<div class="page-wrapper">
		...
		<div class="row">
			<div class="col-md-12">
				<?php
					$linkRegistrasiType = "link_panitia";
					$linkRegistrasi = "";
					$linkRegistrasiOn = "";
					...
					$linkRegisUrl = base_url("/kegiatan/registrasi_panitia/".$kegiatan["id"]);
					...
					<div class="switch d-inline generateBitlyLinkBtn">
						<input type="checkbox" id="generateLinkRegistrasi" '.$linkRegistrasiOn.' data-type="panitia" value="'.$kegiatan["id"].'" />
					...
					$this->bootgrid->setTable("kegiatan_panitia", $conditions);
					$this->bootgrid->setTitle("PANITIA ".$turnOnRegisterButton);
					$this->bootgrid->sortBy("dibuat_tgl");
					$this->bootgrid->sortType("ASC");

					$cardLink = array();
					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_biodata/".$kegiatan["id"]."/panitia",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Biodata"
					);
					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_daftar_hadir/".$kegiatan["id"]."/panitia",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Daftar Hadir"
					);
					$cardLink[] = array(
						"target" => "/admin/kegiatan/export_excel/".$kegiatan["id"]."/panitia",
					...
					$columns[] = array(
						...
						"link" => array(
							"url" => "/admin/kegiatan/download_single_biodata/panitia/{{id}}",
							"target" => "_blank"
						),
						"visible" => "false"
					);
					...
					$showSurtug = "false";
					if (!empty($kegiatan["form_upload_surtug_panitia"]) || !empty($kegiatan["form_wajib_surtug_panitia"]))  {
						$showSurtug = "true";
					}
					...
					if ($this->utility->hasUserAccess("panitia_kegiatan","add")) {
						$addButton = array(
							"text" => "Tambah",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("panitia_kegiatan","edit")) {
						$editButton = array(
							"text" => "Edit",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("panitia_kegiatan","delete")) {
						$deleteButton = array(
							"text" => "Delete"
						);
