...
<div class="main-body">
	<div class="page-wrapper">
		...
		<div class="row">
			<div class="col-md-12">
				<?php
					$linkRegistrasiType = "link_narasumber";
					$linkRegistrasi = "";
					$linkRegistrasiOn = "";
					...
					$linkRegisUrl = base_url("/kegiatan/registrasi_narasumber/".$kegiatan["id"]);
					...
					<div class="switch d-inline generateBitlyLinkBtn">
						<input type="checkbox" id="generateLinkRegistrasi" '.$linkRegistrasiOn.' data-type="narasumber" value="'.$kegiatan["id"].'" />
					...
					$this->bootgrid->setTable("kegiatan_narasumber", $conditions);
					$this->bootgrid->setTitle("NARASUMBER ".$turnOnRegisterButton);
					$this->bootgrid->sortBy("dibuat_tgl");
					$this->bootgrid->sortType("ASC");

					$cardLink = array();
					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_biodata/".$kegiatan["id"]."/narasumber",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Biodata"
					);

					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_daftar_hadir/".$kegiatan["id"]."/narasumber",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Daftar Hadir"
					);

					$cardLink[] = array(
						"target" => "/admin/kegiatan/export_excel/".$kegiatan["id"]."/narasumber",
					...
					$columns[] = array(
						...
						"link" => array(
							"url" => "/admin/kegiatan/download_single_biodata/narasumber/{{id}}",
							"target" => "_blank"
						),
						"visible" => "false"
					);
					...
					$showSurtug = "false";
					if (!empty($kegiatan["form_upload_surtug_narasumber"]) || !empty($kegiatan["form_wajib_surtug_narasumber"]))  {
						$showSurtug = "true";
					}
					...
					if ($this->utility->hasUserAccess("narasumber_kegiatan","add")) {
						$addButton = array(
							"text" => "Tambah",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("narasumber_kegiatan","edit")) {
						$editButton = array(
							"text" => "Edit",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("narasumber_kegiatan","delete")) {
						$deleteButton = array(
							"text" => "Delete"
						);
