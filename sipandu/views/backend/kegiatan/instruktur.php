...
<div class="main-body">
	<div class="page-wrapper">
		...
		<div class="row">
			<div class="col-md-12">
				<?php
					$linkRegistrasiType = "link_instruktur";
					$linkRegistrasi = "";
					$linkRegistrasiOn = "";
					...
					$linkRegisUrl = base_url("/kegiatan/registrasi_instruktur/".$kegiatan["id"]);
					...
					<div class="switch d-inline generateBitlyLinkBtn">
						<input type="checkbox" id="generateLinkRegistrasi" '.$linkRegistrasiOn.' data-type="instruktur" value="'.$kegiatan["id"].'" />
					...
					$this->bootgrid->setTable("kegiatan_instruktur", $conditions);
					$this->bootgrid->setTitle("INSTRUKTUR ".$turnOnRegisterButton);
					$this->bootgrid->sortBy("dibuat_tgl");
					$this->bootgrid->sortType("ASC");

					$cardLink = array();
					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_biodata/".$kegiatan["id"]."/instruktur",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Biodata"
					);

					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_daftar_hadir/".$kegiatan["id"]."/instruktur",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Daftar Hadir"
					);

					$cardLink[] = array(
						"target" => "/admin/kegiatan/export_excel/".$kegiatan["id"]."/instruktur",
					...
					$columns[] = array(
						...
						"link" => array(
							"url" => "/admin/kegiatan/download_single_biodata/instruktur/{{id}}",
							"target" => "_blank"
						),
						"visible" => "false"
					);
					...
					$showSurtug = "false";
					if (!empty($kegiatan["form_upload_surtug_instruktur"]) || !empty($kegiatan["form_wajib_surtug_instruktur"]))  {
						$showSurtug = "true";
					}
					...
					if ($this->utility->hasUserAccess("instruktur_kegiatan","add")) {
						$addButton = array(
							"text" => "Tambah",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("instruktur_kegiatan","edit")) {
						$editButton = array(
							"text" => "Edit",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("instruktur_kegiatan","delete")) {
						$deleteButton = array(
							"text" => "Delete"
						);
