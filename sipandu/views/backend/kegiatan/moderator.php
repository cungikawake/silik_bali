...
<div class="main-body">
	<div class="page-wrapper">
		...
		<div class="row">
			<div class="col-md-12">
				<?php
					$linkRegistrasiType = "link_moderator";
					$linkRegistrasi = "";
					$linkRegistrasiOn = "";
					...
					$linkRegisUrl = base_url("/kegiatan/registrasi_moderator/".$kegiatan["id"]);
					...
					<div class="switch d-inline generateBitlyLinkBtn">
						<input type="checkbox" id="generateLinkRegistrasi" '.$linkRegistrasiOn.' data-type="moderator" value="'.$kegiatan["id"].'" />
					...
					$this->bootgrid->setTable("kegiatan_moderator", $conditions);
					$this->bootgrid->setTitle("MODERATOR ".$turnOnRegisterButton);
					$this->bootgrid->sortBy("dibuat_tgl");
					$this->bootgrid->sortType("ASC");

					$cardLink = array();
					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_biodata/".$kegiatan["id"]."/moderator",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Biodata"
					);

					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_daftar_hadir/".$kegiatan["id"]."/moderator",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Daftar Hadir"
					);

					$cardLink[] = array(
						"target" => "/admin/kegiatan/export_excel/".$kegiatan["id"]."/moderator",
					...
					$columns[] = array(
						...
						"link" => array(
							"url" => "/admin/kegiatan/download_single_biodata/moderator/{{id}}",
							"target" => "_blank"
						),
						"visible" => "false"
					);
					...
					$showSurtug = "false";
					if (!empty($kegiatan["form_upload_surtug_moderator"]) || !empty($kegiatan["form_wajib_surtug_moderator"]))  {
						$showSurtug = "true";
					}
					...
					if ($this->utility->hasUserAccess("moderator_kegiatan","add")) {
						$addButton = array(
							"text" => "Tambah",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("moderator_kegiatan","edit")) {
						$editButton = array(
							"text" => "Edit",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("moderator_kegiatan","delete")) {
						$deleteButton = array(
							"text" => "Delete"
						);
