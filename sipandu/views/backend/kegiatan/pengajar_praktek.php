		...
		<div class="row">
			<div class="col-md-12">
				<?php
					...
					$linkRegisUrl = base_url("/kegiatan/registrasi_pengajar_praktek/".$kegiatan["id"]);
					...
					<div class="switch d-inline generateBitlyLinkBtn">
						<input type="checkbox" id="generateLinkRegistrasi" '.$linkRegistrasiOn.' data-type="pengajar_praktek" value="'.$kegiatan["id"].'" />
					...
					$this->bootgrid->setTable("kegiatan_pengajar_praktek", $conditions);
					$this->bootgrid->setTitle("PENGAJAR PRAKTEK ".$turnOnRegisterButton);
					$this->bootgrid->sortBy("dibuat_tgl");
					$this->bootgrid->sortType("ASC");

					$cardLink = array();
					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_biodata/".$kegiatan["id"]."/pengajar_praktek",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Biodata"
					);

					$cardLink[] = array(
						"target" => "/admin/kegiatan/download_daftar_hadir/".$kegiatan["id"]."/pengajar_praktek",
						"icon" => "fas fa-file-pdf",
						"text" => "Download Daftar Hadir"
					);

					$cardLink[] = array(
						"target" => "/admin/kegiatan/export_excel/".$kegiatan["id"]."/pengajar_praktek",
					...
					$columns[] = array(
						...
						"link" => array(
							"url" => "/admin/kegiatan/download_single_biodata/pengajar_praktek/{{id}}",
							"target" => "_blank"
						),
						"visible" => "false"
					);
					...
					$showSurtug = "false";
					if (!empty($kegiatan["form_upload_surtug_pp"]) || !empty($kegiatan["form_wajib_surtug_pp"]))  {
						$showSurtug = "true";
					}
					...
					if ($this->utility->hasUserAccess("pengajar_praktek_kegiatan","add")) {
						$addButton = array(
							"text" => "Tambah",
							"parent" => $kegiatan["id"],
							"modal" => array(
								"view" => "backend/kegiatan/modal_kegiatan_item",
								"data" => array(
									"table" => "kegiatan_pengajar_praktek"
								)
							)
						);
					...
					if ($this->utility->hasUserAccess("pengajar_praktek_kegiatan","edit")) {
						$editButton = array(
							"text" => "Edit",
							"parent" => $kegiatan["id"],
							"modal" => array(
					...
					if ($this->utility->hasUserAccess("pengajar_praktek_kegiatan","delete")) {
						$deleteButton = array(
							"text" => "Delete"
						);
