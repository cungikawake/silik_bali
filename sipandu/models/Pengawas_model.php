...
class Pengawas_model extends CI_Model{
	...
	public function get ($kegiatan, $nik) {
		...
		$this->db->select("*");
		$this->db->from("kegiatan_pengawas");
	...
	public function save ($data, $id = 0) {
		...
		if (empty($id)) {
			...
			if (!isset($kegiatan["no_urut_terakhir"]["pengawas"])) {
				$kegiatan["no_urut_terakhir"]["pengawas"] = 1;
			}
			...
			$this->db->insert("kegiatan_pengawas", $data);
			$id = $this->db->insert_id();
			$this->db->reset_query();
			
			$kode = array();
			$kode["kode"] = $this->utility->penomoran($kegiatan["no_urut_terakhir"]["pengawas"])."-PW-".$kegiatan["kode"];
			
			$this->db->where("id", $id);
			$this->db->update("kegiatan_pengawas", $kode);
			$this->db->reset_query();
			
			$kegiatan["no_urut_terakhir"]["pengawas"] = $kegiatan["no_urut_terakhir"]["pengawas"] + 1;
			...
			$this->db->where("id", $id);
			$this->db->update("kegiatan_pengawas", $data);
			$this->db->reset_query();
		}
	...
	public function delete ($id) {
		$this->db->where('id', $id);
		$this->db->delete('kegiatan_pengawas');
		$this->db->reset_query();
	}
}