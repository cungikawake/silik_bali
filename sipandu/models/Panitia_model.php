...
class Panitia_model extends CI_Model{
	...
	public function getPanitia ($kegiatan, $nik) {
		$out = array();
		
		$this->db->select("*");
		$this->db->from("kegiatan_panitia");
	...
	public function getPanitiaByKode ($kode) {
		$out = array();
	...
	public function getPanitiaByNik ($nik) {
		$out = array();
	...
	public function getPanitiaKegiatan ($kegiatan, $sortKab = false) {
		$out = array();
	...
	public function save ($data, $id = 0) {
		...
		if (empty($id)) {
			...
			if (!isset($kegiatan["no_urut_terakhir"]["panitia"])) {
				$kegiatan["no_urut_terakhir"]["panitia"] = 1;
			}
			...
			$this->db->insert("kegiatan_panitia", $data);
			$id = $this->db->insert_id();
			$this->db->reset_query();
			
			$kode = array();
			$kode["kode"] = $this->utility->penomoran($kegiatan["no_urut_terakhir"]["panitia"])."-PT-".$kegiatan["kode"];
			$this->db->where("id", $id);
			$this->db->update("kegiatan_panitia", $kode);
			$this->db->reset_query();
			
			$kegiatan["no_urut_terakhir"]["panitia"] = $kegiatan["no_urut_terakhir"]["panitia"] + 1;
			...
			$this->db->where("id", $id);
			$this->db->update("kegiatan_panitia", $data);
			$this->db->reset_query();
		}
	...
	public function delete ($id) {
		$this->db->where('id', $id);
		$this->db->delete('kegiatan_panitia');
		$this->db->reset_query();
	}
}