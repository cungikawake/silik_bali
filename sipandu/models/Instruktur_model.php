...
class Instruktur_model extends CI_Model{
	...
	public function get ($kegiatan, $nik) {
		...
		$this->db->select("*");
		$this->db->from("kegiatan_instruktur");
	...
	public function save ($data, $id = 0) {
		...
		if (empty($id)) {
			...
			if (!isset($kegiatan["no_urut_terakhir"]["instruktur"])) {
				$kegiatan["no_urut_terakhir"]["instruktur"] = 1;
			}
			...
			$this->db->insert("kegiatan_instruktur", $data);
			$id = $this->db->insert_id();
			$this->db->reset_query();
			
			$kode = array();
			$kode["kode"] = $this->utility->penomoran($kegiatan["no_urut_terakhir"]["instruktur"])."-IN-".$kegiatan["kode"];
			$this->db->where("id", $id);
			$this->db->update("kegiatan_instruktur", $kode);
			$this->db->reset_query();
			
			$kegiatan["no_urut_terakhir"]["instruktur"] = $kegiatan["no_urut_terakhir"]["instruktur"] + 1;
			...
			$this->db->where("id", $id);
			$this->db->update("kegiatan_instruktur", $data);
			$this->db->reset_query();
		}
	...
	public function delete ($id) {
		$this->db->where('id', $id);
		$this->db->delete('kegiatan_instruktur');
		$this->db->reset_query();
	}
}