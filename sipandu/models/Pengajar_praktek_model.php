...
class Pengajar_praktek_model extends CI_Model{
	...
	public function get ($kegiatan, $nik) {
		...
		$this->db->select("*");
		$this->db->from("kegiatan_pengajar_praktek");
	...
	public function save ($data, $id = 0) {
		...
		if (empty($id)) {
			...
			if (!isset($kegiatan["no_urut_terakhir"]["pengajar_praktek"])) {
				$kegiatan["no_urut_terakhir"]["pengajar_praktek"] = 1;
			}
			...
			$this->db->insert("kegiatan_pengajar_praktek", $data);
			$id = $this->db->insert_id();
			$this->db->reset_query();
			
			$kode = array();
			$kode["kode"] = $this->utility->penomoran($kegiatan["no_urut_terakhir"]["pengajar_praktek"])."-PP-".$kegiatan["kode"];
			$this->db->where("id", $id);
			$this->db->update("kegiatan_pengajar_praktek", $kode);
			$this->db->reset_query();
			
			$kegiatan["no_urut_terakhir"]["pengajar_praktek"] = $kegiatan["no_urut_terakhir"]["pengajar_praktek"] + 1;
			...
			$this->db->where("id", $id);
			$this->db->update("kegiatan_pengajar_praktek", $data);
			$this->db->reset_query();
		}
	...
	public function delete ($id) {
		$this->db->where('id', $id);
		$this->db->delete('kegiatan_pengajar_praktek');
		$this->db->reset_query();
	}
}