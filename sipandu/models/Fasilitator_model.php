...
class Fasilitator_model extends CI_Model{
	...
	public function get ($kegiatan, $nik) {
		...
		$this->db->select("*");
		$this->db->from("kegiatan_fasilitator");
	...
	public function save ($data, $id = 0) {
		...
		if (empty($id)) {
			...
			if (!isset($kegiatan["no_urut_terakhir"]["fasilitator"])) {
				$kegiatan["no_urut_terakhir"]["fasilitator"] = 1;
			}
			...
			$this->db->insert("kegiatan_fasilitator", $data);
			$id = $this->db->insert_id();
			$this->db->reset_query();
			
			$kode = array();
			$kode["kode"] = $this->utility->penomoran($kegiatan["no_urut_terakhir"]["fasilitator"])."-FS-".$kegiatan["kode"];
			
			$this->db->where("id", $id);
			$this->db->update("kegiatan_fasilitator", $kode);
			$this->db->reset_query();
			
			$kegiatan["no_urut_terakhir"]["fasilitator"] = $kegiatan["no_urut_terakhir"]["fasilitator"] + 1;
			...
			$this->db->where("id", $id);
			$this->db->update("kegiatan_fasilitator", $data);
			$this->db->reset_query();
		}
	...
	public function delete ($id) {
		$this->db->where('id', $id);
		$this->db->delete('kegiatan_fasilitator');
		$this->db->reset_query();
	}
}