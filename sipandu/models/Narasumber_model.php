...
class Narasumber_model extends CI_Model{
	...
	public function getNarasumber ($kegiatan, $nik) {
		$out = array();
		
		$this->db->select("*");
		$this->db->from("kegiatan_narasumber");
	...
	public function getNarasumberById ($id) {
		$out = array();
	...
	public function getNarasumberByKode ($kode) {
		$out = array();
	...
	public function getNarasumberByNik ($nik) {
		$out = array();
	...
	public function getNarasumberKegiatan ($kegiatan, $sortKab = false) {
		$out = array();
	...
	public function save ($data, $id = 0) {
		...
		if (empty($id)) {
			...
			if (!isset($kegiatan["no_urut_terakhir"]["narasumber"])) {
				$kegiatan["no_urut_terakhir"]["narasumber"] = 1;
			}
			...
			$this->db->insert("kegiatan_narasumber", $data);
			$id = $this->db->insert_id();
			$this->db->reset_query();
			
			$kode = array();
			$kode["kode"] = $this->utility->penomoran($kegiatan["no_urut_terakhir"]["narasumber"])."-NS-".$kegiatan["kode"];
			$this->db->where("id", $id);
			$this->db->update("kegiatan_narasumber", $kode);
			$this->db->reset_query();
			
			$kegiatan["no_urut_terakhir"]["narasumber"] = $kegiatan["no_urut_terakhir"]["narasumber"] + 1;
			...
			$this->db->where("id", $id);
			$this->db->update("kegiatan_narasumber", $data);
			$this->db->reset_query();
		}
	...
	public function delete ($id) {
		$this->db->where('id', $id);
		$this->db->delete('kegiatan_narasumber');
		$this->db->reset_query();
	}
}