...
class Moderator_model extends CI_Model{
	...
	public function get ($kegiatan, $nik) {
		...
		$this->db->select("*");
		$this->db->from("kegiatan_moderator");
	...
	public function save ($data, $id = 0) {
		...
		if (empty($id)) {
			...
			if (!isset($kegiatan["no_urut_terakhir"]["moderator"])) {
				$kegiatan["no_urut_terakhir"]["moderator"] = 1;
			}
			...
			$this->db->insert("kegiatan_moderator", $data);
			$id = $this->db->insert_id();
			$this->db->reset_query();
			
			$kode = array();
			$kode["kode"] = $this->utility->penomoran($kegiatan["no_urut_terakhir"]["moderator"])."-MD-".$kegiatan["kode"];
			
			$this->db->where("id", $id);
			$this->db->update("kegiatan_moderator", $kode);
			$this->db->reset_query();
			
			$kegiatan["no_urut_terakhir"]["moderator"] = $kegiatan["no_urut_terakhir"]["moderator"] + 1;
			...
			$this->db->where("id", $id);
			$this->db->update("kegiatan_moderator", $data);
			$this->db->reset_query();
		}
	...
	public function delete ($id) {
		$this->db->where('id', $id);
		$this->db->delete('kegiatan_moderator');
		$this->db->reset_query();
	}
}