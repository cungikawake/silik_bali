...
class Spj_narasumber_model extends CI_Model{
	...
	public function getById ($id) {
		...
		$this->db->select("*");
		$this->db->from("spj_kegiatan_narasumber");
		$this->db->where("id", $id);
	...
	public function save ($data, $id = 0) {
		if (empty($id)) {
			...
			$this->db->insert("spj_kegiatan_narasumber", $data);
			...
			$this->db->where("id", $id);
			$this->db->update("spj_kegiatan_narasumber", $data);
		}
	...
	public function deleteBySpjId ($spjId, $lock = "0") {
		...
		$this->db->delete('spj_kegiatan_narasumber');
		$this->db->reset_query();
